...
def conv2d(x, fmaps, kernel, gain=np.sqrt(2), use_wscale=False):
    ...
    tmp = tf.nn.conv2d(x_, w, strides=[1,1,1,1], padding='SAME', data_format='NHWC')
    tmp_ = tf.transpose(tmp, [0, 3, 1, 2])
...
def upscale2d_conv2d(x, fmaps, kernel, gain=np.sqrt(2), use_wscale=False):
    ...
    tmp = tf.nn.conv2d_transpose(x_, w, os, strides=[1,2,2,1], padding='SAME', data_format='NHWC')
    tmp_ = tf.transpose(tmp, [0, 3, 1, 2])
    return tmp_
#----------------------------------------------------------------------------
...
def downscale2d(x, factor=2):
    ...
    with tf.variable_scope('Downscale2D'):
        ...
        x_ = tf.transpose(x, [0, 2, 3, 1])
        tmp = tf.nn.avg_pool(x_, ksize=ksize, strides=ksize, padding='VALID', data_format='NHWC') # NOTE: requires tf_config['graph_options.place_pruned_graph'] = True
        tmp_ = tf.transpose(tmp, [0, 3, 1, 2])
        return tmp_
#----------------------------------------------------------------------------
...
def conv2d_downscale2d(x, fmaps, kernel, gain=np.sqrt(2), use_wscale=False):
    ...
    x_ = tf.transpose(x, [0, 2, 3, 1])
    tmp = tf.nn.conv2d(x_, w, strides=[1,2,2,1], padding='SAME', data_format='NHWC')
    tmp_ = tf.transpose(tmp, [0, 3, 1, 2])
    return tmp_
